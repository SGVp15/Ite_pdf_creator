...
from config import PAGE_NAME, FILE_XLSX, dictory, confirm_docx, print_docx
...
def get_contact_from_excel(rows_excel, templates_docx) -> list:
    ...
    for template in templates_docx:
        for i in rows_excel:
            ...
            if os.path.exists('./output/pdf') == False:
                os.mkdir('./output/pdf')

            if os.path.exists('./output/docx') == False:
                os.mkdir('./output/docx')

            for _path in ('pdf', 'docx'):
                path_folder = f'./output/{_path}/{dir_name}'
                if os.path.exists(path_folder) == False:
                    os.mkdir(f'./output/{_path}/{dir_name}')
